...
title("Courbe de données pour la fonction linéaire de FA")
subplot(2,1,2)
plot((YB*FB+FC*YC+Fg_sphere.*Py)/inertieP)
title("Courbe de données du banc d'essaie")

figure()
hold on
subplot(2,1,1)
acc_Ax = delta_ddAy(1).*(Pz - Pz_e) + delta_ddAy(2).*(Ay) + delta_ddAy(3).*(Ax) + delta_ddAy(4).*(Py) + delta_ddAy(5).*(IB - Ib_e) + delta_ddAy(6).*(IC - Ic_e) ;
plot(acc_Ax)
title("Courbe de données pour la fonction linéaire de FA")
subplot(2,1,2)
plot((YB*FB+FC*YC+Fg_sphere.*Py)/inertieP)
